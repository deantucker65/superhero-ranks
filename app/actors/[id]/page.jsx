...
export default function AdminPage() {
  ...
  async function loadCharactersForActor(actorId, setter) {
  ...
  async function updateCharacter(e) {
    ...
    if (error) {
      ...
      loadCharactersForActor(editActorId, setEditCharacters)
    }
  }
  ...
  async function deleteCharacter() {
    ...
    if (error) {
      ...
      loadCharactersForActor(deleteActorId, setDeleteCharacters)
    }
  }
          ...
          <form onSubmit={addActor} className="space-y-4">
            <div>
              ...
              <input type="text" value={actorName} onChange={(e) => setActorName(e.target.value)}
                className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white" required />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Bio (optional)</label>
              <textarea value={actorBio} onChange={(e) => setActorBio(e.target.value)}
                className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white" rows={3} />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Actor Photo URL (optional)</label>
              <input type="text" value={actorPhotoUrl} onChange={(e) => setActorPhotoUrl(e.target.value)}
        ...
        <div className="bg-gray-800 rounded-2xl p-6 mb-8">
          ...
          <form onSubmit={addCharacter} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={universe} onChange={(e) => setUniverse(e.target.value)}
                className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white" placeholder="e.g. Marvel, DC, Dark Horse" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Power Tier</label>
              <select value={powerTier} onChange={(e) => setPowerTier(e.target.value)}
            ...
            <div>
              ...
              <input type="text" value={characterPhotoUrl} onChange={(e) => setCharacterPhotoUrl(e.target.value)}
                className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white" placeholder="https://..." />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Powers & Abilities</label>
              <textarea value={powers} onChange={(e) => setPowers(e.target.value)}
                className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white" rows={3}
                placeholder="e.g. Hellfire manipulation, superhuman strength..." />
            </div>
            <div>
        ...
        <div className="bg-gray-800 rounded-2xl p-6 mb-8">
          ...
          <div className="space-y-4 mb-6">
            <div>
              ...
              <select value={editActorId} onChange={(e) => {
                setEditActorId(e.target.value)
                loadCharactersForActor(e.target.value, setEditCharacters)
                setEditMode(false)
              }} className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white">
                <option value="">Select an actor...</option>
                {actors.map((actor) => (
            ...
            {editCharacters.length > 0 && (
              <div>
                ...
                <select value={editCharacterId} onChange={(e) => {
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={deleteActorId} onChange={(e) => {
                ...
                setDeleteCharacterId('')
                if (e.target.value) loadCharactersForActor(e.target.value, setDeleteCharacters)
                else setDeleteCharacters([])
              }} className="w-full bg-gray-700 rounded-lg px-4 py-2 text-white">
                <option value="">Select an actor...</option>
                {actors.map((actor) => (
                  <option key={actor.id} value={actor.id}>{actor.name}</option>
